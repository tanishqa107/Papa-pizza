...
const MenuItem: React.FC<MenuItemProps> = ({ pizza, addToCart}) => {
      ...
      <div className="p-4 pt-0 mt-auto">
        <div className="flex justify-between items-center mt-4">
          <select
            ...
            className="border border-gray-300 rounded px-3 py-2 mr-2 text-sm font-semibold text-stone-800 bg-stone-100 hover:bg-stone-200 transition-colors"
          >
            <option value="small">Small - ₹{pizza.sizes?.small?.toFixed(2) ?? 'N/A'}</option>
            <option value="medium">Medium - ₹{pizza.sizes?.medium?.toFixed(2) ?? 'N/A'}</option>
            <option value="large">Large - ₹{pizza.sizes?.large?.toFixed(2) ?? 'N/A'}</option>
          </select>
          <PizzaButton
